...
const PricingPlan: React.FC<
  PricingPlanProps & { billingCycle: "monthly" | "yearly" }
> = ({
  name,
  price,
  description,
  ...
  return (
    <Card
      className={`border h-full ${isPopular ? "border-primary shadow-md relative" : "shadow-sm"}`}
    >
      {isPopular && (
      ...
      <CardHeader className="pt-8 px-6">
        ...
        <div className="mt-4">
          <span className="text-3xl font-bold">
            ₽{price[billingCycle].toLocaleString()}
          </span>
        ...
        <ul className="space-y-3">
          {features.map((feature, index) => (
            <li key={index} className="flex items-start">
              <div
                className={`mt-1 mr-3 ${feature.included ? "text-green-500" : "text-gray-300"}`}
              >
                <Icon
  ...
  const pricingPlans: PricingPlanProps[] = [
    {
      ...
      description:
        "Идеально для начинающих, кто хочет попробовать программирование",
      features: [
    ...
    },
    {
      ...
      description:
        "Наиболее полный курс для серьезного обучения программированию",
      features: [
      ...
      <div className="container">
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          ...
          <p className="text-lg text-secondary/70">
            Выберите тариф, который подходит именно вам. Все тарифы включают
        ...
        <Tabs defaultValue="monthly" className="w-full max-w-5xl mx-auto">
          ...
          <TabsContent value="monthly" className="space-y-8">
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {pricingPlans.map((plan, index) => (
                <PricingPlan key={index} {...plan} billingCycle="monthly" />
              ))}
            </div>
          </TabsContent>
          ...
          <TabsContent value="yearly" className="space-y-8">
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {pricingPlans.map((plan, index) => (
                <PricingPlan key={index} {...plan} billingCycle="yearly" />
              ))}
            </div>
          </TabsContent>
        </Tabs>
        ...
        <div className="mt-16 text-center max-w-3xl mx-auto">
          <div className="bg-gray-50 p-6 rounded-lg border border-gray-100">
            ...
            <p className="text-secondary/70 mb-5">
              Если у вас есть вопросы о курсе или вы хотите узнать больше о
              корпоративных тарифах, свяжитесь с нашей командой.
            </p>
